...
import 'package:moneynest/core/database/tables/budget_table.dart';
...
@DriftDatabase(
  tables: [Categories, Transactions, Wallets, Budgets],
  daos: [CategoryDao, TransactionDao, WalletDao, BudgetDao],
)
class AppDatabase extends _$AppDatabase {
  AppDatabase([QueryExecutor? executor]) : super(executor ?? _openConnection());

  @override
  int get schemaVersion => 11; // Tăng schema version lên 1 đơn vị
  ...
  MigrationStrategy get migration {
    return MigrationStrategy(
      ...
      onUpgrade: (Migrator m, int from, int to) async {
        ...
        // Thêm cột used_amount vào budgets nếu chưa có
        if (from < 11) {
          await m.addColumn(budgets, budgets.usedAmount);
        }
        // Thêm cột currentBalance vào budgets nếu chưa có
        if (from < 12) {
          await m.addColumn(budgets, budgets.currentBalance);
        }
      },
    );
  }
